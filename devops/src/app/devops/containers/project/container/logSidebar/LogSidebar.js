...
import { Select, Modal, Button } from 'choerodon-ui';
...
export default class LogSidebar extends Component {
  ...
  componentDidMount() {
    const { store, current: { id } } = this.props;
    const { projectId } = AppState.currentMenuType;

    store.loadPodParam(projectId, id).then(data => {
      if (data && data.length) {
  ...
  render() {
    ...
    const { following, fullScreen, containerName, podName, containers } = this.state;
